...
import { View, Text, StyleSheet, FlatList, Alert, TextInput, Pressable } from "react-native";
import { Picker } from "@react-native-picker/picker";
import { useBranchAllProductList, useBranchName, useTransferQuantity, useBackInventoryProductList } from "@/src/api/products"; // Adjust the import path accordingly
...
const Details = () => {
  ...
  const { data: backInventoryProducts } = useBackInventoryProductList(id_branch.toString());
  ...
  useEffect(() => {
    if (products) {
      const filtered = products.filter((item: any) => {
        ...
        const matchesCategory = selectedCategory ? item.category.categoryName === selectedCategory : true;
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: "Confirm",
          onPress: () => {
            Object.entries(restockQuantities).forEach(([id_products, quantity]) => {
              transferQuantity({
  ...
  return (
    <View style={styles.container}>
        ...
        <>
          ...
          <View style={styles.filterContainer}>
            ...
            >
              ...
              {products && [...new Set(products.map((item: any) => item.category.categoryName))].map((category) => (
          ...
          <FlatList
            ...
            keyExtractor={(item: any) => item.id_products?.toString() || item.id.toString()}
          />
          ...
          <Button text="Restock" onPress={() => setRestockModalVisible(true)} style={styles.restockButton} />
        </>
      ) : (
      ...
      <RestockModal
        ...
        allItems={products?.map((product: any) => ({
          ...product,
          backInventoryQuantity: (backInventoryProducts?.find((item: any) => item.id_products === product.id_products) as { quantity: number } | undefined)?.quantity || 0,
        })) || []}
